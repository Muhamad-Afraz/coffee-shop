        ...
        <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
          ...
          <div className="md:col-span-3 md:col-start-7">
            ...
            <p className="mt-4 font-serif text-lg leading-snug text-start">
              {t("visit.address")}<br />
              {t("visit.hours")?.map((h: { label: string; time: string }, i: number) => <span key={i}>{h.label}: {h.time}<br /></span>)}
            </p>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <ul className="mt-4 space-y-2 font-serif text-lg">
              {social.map((s) => (
                <li key={s}><a href={s === "Instagram" ? "https://www.instagram.com/fairenoughcoffee/" : s === "TikTok" ? "https://www.tiktok.com/discover/fair-enough-coffee-%7C-%D9%82%D9%87%D9%88%D8%A9-%D9%81%D9%8A%D8%B1-%D8%A7%D9%8A%D9%86%D9%81" : "#"} target={s === "Instagram" || s === "TikTok" ? "_blank" : undefined} rel={s === "Instagram" || s === "TikTok" ? "noreferrer" : undefined} className="hover:text-gold transition-colors">{s}</a></li>
              ))}
            </ul>
          </div>
        </div>
